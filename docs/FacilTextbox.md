...
myTextbox = FacilTextbox_class.new( (Will appear at bottom?, a boolean), (Speed, int recommended), (lines, a table of strings), (red value, int recommended), (green value, int recommended), (blue value, int recommended), (Alpha value, a number) )
```
...
- setIn(boolean)
- :setAtBottom(bool)
- :changeColor(red value, green value, blue value, alpha value) [Changes color of the text.]
- :modifyName(red value, green value, blue value, alpha value, new Speaker) [Changes color of the speaker label and allows you to remove it by setting it to removeName. If no speaker was set, adding a name will set it.]
- :setPitch(pitch) [High or low. You may want to add your own sounds though. So feel free to modify whatever you need.]
